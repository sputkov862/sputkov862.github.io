...
type Bubble = {
  ...
  color: PaletteColor;
  colorFrom: PaletteColor;
  colorTo: PaletteColor;
  colorT: number;
  colorTick: number;
  solid: boolean;
  ghostUntil: number;
...
type HudRefs = {
  gbValueEl: HTMLElement;
  fpsValueEl: HTMLElement;
  fpsMinEl: HTMLElement;
  fpsAvgEl: HTMLElement;
  fpsMaxEl: HTMLElement;
  ...
  collisionsToggle: HTMLButtonElement;
  logEl: HTMLElement;
};
...
class BubbleApp {
  ...
  private width = 0;
  private height = 0;
  private backgroundFill: CanvasGradient | null = null;
  ...
  private readonly palette: PaletteColor[] = ['#ff2f56', '#ff7a00', '#ffd200', '#7be000', '#25d6ff', '#2b7bff', '#8a2be2', '#ff5be0'];
  private readonly bubbles: Bubble[] = [];
  private readonly MAX_BUBBLES = 60;
  private readonly baseTexture = new Image();
  private textureReady = false;
  private readonly neighborOffsets: Array<[number, number]> = [
    [0, 0],
    [1, 0],
    [-1, 0],
    [0, 1],
  ...
  private readonly SPAWN_INTERVAL_MS = 90;
  private readonly SPAWN_BATCH = 1;

  private bubbleIdSeq = 0;
  private desiredSizes: number[] = [];

  private readonly BASE_RADIUS = 26;
  private readonly AREA_LIMIT = 0.55;
  private readonly PAD_AREA_COEFF = 8;
  private cappedGB = 0;

  private readonly MAX_VEL = 2.2;
  private readonly COLLISION_ITERS = 2;

  private lastConsoleLog = 0;

  private readonly grid = new Map<string, Bubble[]>();
  private readonly arrayPool: Bubble[][] = [];
  private readonly cellSize = 70;
  private readonly checkedPairs = new Set<string>();

  private overlapsCount = 0;
  ...
  private fpsMax = 0;
  private fpsTotalFrames = 0;
  private fpsTotalMs = 0;
  private lastLogUpdate = 0;
  private ghostsCount = 0;
  private readonly pendingSpawns: number[] = [];
  private spawnCooldown = 0;
  private lastSpawnTick = performance.now();
  private lastFrameTime = performance.now();

  constructor(private readonly hud: HudRefs) {
  ...
  private acquireArray(): Bubble[] {
    return this.arrayPool.pop() || [];
  }

  private releaseArray(arr: Bubble[]) {
    arr.length = 0;
  ...
  private tintSprite(color: PaletteColor, target?: HTMLCanvasElement | null): HTMLCanvasElement | null {
    if (!this.textureReady) return null;
  ...
  private rgbToHex(r: number, g: number, b: number): PaletteColor {
    const toHex = (v: number) => v.toString(16).padStart(2, '0');
    return `#${toHex(r)}${toHex(g)}${toHex(b)}`;
  }

  private lerpColor(a: PaletteColor, b: PaletteColor, t: number): PaletteColor {
  ...
  private pickPaletteColor(except: PaletteColor): PaletteColor {
    let c = except;
    while (c === except) {
  ...
  private makeBubble(spawnFromCenter = false, sizeGB = 1): Bubble {
    const depth = 1;
  ...
  private planSizes(totalGB: number) {
    const sizes: number[] = [];
  ...
  private enqueueSpawn(size: number) {
    this.pendingSpawns.push(size);
  }

  private spawnQueued(now: number) {
    this.spawnCooldown -= now - this.lastSpawnTick;
    this.lastSpawnTick = now;
    if (this.spawnCooldown > 0 || this.pendingSpawns.length === 0) return;
  ...
  private rebuildGrid() {
    for (const arr of this.grid.values()) this.releaseArray(arr);
    this.grid.clear();
  ...
  private handleFrame = (now: number) => {
  ...
  private resolveCollisionsSpatial() {
      ...
      for (const [dx, dy] of this.neighborOffsets) {
        ...
        for (let i = 0; i < list.length; i++) {
          ...
          for (let j = 0; j < otherList.length; j++) {
            ...
            if (this.checkedPairs.has(pairId)) continue;
  ...
  private updateLog(now: number) {
    if (now - this.lastLogUpdate < 300) return;
...
function queryRequired<T extends Element>(selector: string, type: new () => T): T {
